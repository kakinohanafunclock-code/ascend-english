...
## 6.5 英単語学習 / 単語帳（SRS）
- Leitner 方式（box 1–5、復習間隔 1/2/4/8/16 日）。`good`=昇格・先送り、`again`=box1 へ降格・当日再出題。
- 画面: 統計（登録/今日の復習/習得）、フラッシュカード復習、手動追加、一覧・削除。
- 追加経路: 基本デッキ投入、手動入力、各採点画面の語注からワンタップ追加。

## 6.6 学習時間の定義
- 総学習時間 = 完了した各タスクの**想定所要時間 (estimatedMinutes) の合算**。
- Reading/Listening=15分、Speaking=10分、Writing=20分。アプリ起動時間には依存しない。

## 6.7 添削の詳細化
- AI 添削/解説は日本語で返却。`translationJa`（日本語訳）と `glossary`（語注: term/意味/例文）を含む。
- 採点画面で日本語訳を表示。語注は単語帳へ追加可能。Reading は本文の日本語訳トグルを提供。
